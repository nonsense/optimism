...
use crate::{OpTxEnvelope, OpTxType, TxDeposit, TxSdm};
use alloy_consensus::{
...
impl From<TxSdm> for OpTypedTransaction {
...
impl From<OpTxEnvelope> for OpTypedTransaction {
    fn from(envelope: OpTxEnvelope) -> Self {
        match envelope {
            ...
            OpTxEnvelope::Sdm(tx) => Self::Sdm(tx.into_inner()),
...
impl From<OpTypedTransaction> for alloy_rpc_types_eth::TransactionRequest {
    fn from(tx: OpTypedTransaction) -> Self {
        match tx {
            ...
            OpTypedTransaction::Sdm(tx) => tx.into(),
...
impl OpTypedTransaction {
    ...
    /// Return the inner SDM transaction if it exists.
    pub const fn sdm(&self) -> Option<&TxSdm> {
        match self {
            Self::Sdm(tx) => Some(tx),
            _ => None,
        }
    ...
    /// Returns `true` if transaction is SDM transaction.
...
impl RlpEcdsaEncodableTx for OpTypedTransaction {
    ...
    fn eip2718_encode_with_type(&self, signature: &Signature, _ty: u8, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Sdm(tx) => tx.network_encode(out),
    ...
    fn network_encode_with_type(&self, signature: &Signature, _ty: u8, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Sdm(tx) => tx.encode_2718(out),
