...
pub enum OpReceiptEnvelope<T = Log> {
    ...
    /// Receipt envelope with type flag 125, containing an SDM receipt.
...
impl<T> OpReceiptEnvelope<T> {
    ...
    pub fn map_logs<U>(self, f: impl FnMut(T) -> U) -> OpReceiptEnvelope<U> {
        match self {
            ...
            Self::Sdm(r) => OpReceiptEnvelope::Sdm(r.map_logs(f)),
...
impl Decodable2718 for OpReceiptEnvelope {
    fn typed_decode(ty: u8, buf: &mut &[u8]) -> Eip2718Result<Self> {
        match ty.try_into().map_err(|_| Eip2718Error::UnexpectedType(ty))? {
            ...
            OpTxType::Sdm => Ok(Self::Sdm(Decodable::decode(buf)?)),
