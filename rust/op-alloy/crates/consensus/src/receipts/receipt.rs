...
impl<T> OpReceipt<T> {
    ...
    pub fn map_logs<U>(self, f: impl FnMut(T) -> U) -> OpReceipt<U> {
        match self {
            ...
            Self::Sdm(receipt) => OpReceipt::Sdm(receipt.map_logs(f)),
        ...
        T: Encodable,
    {
        match self {
            ...
            Self::Eip1559(receipt) |
            Self::Eip7702(receipt) |
            Self::Sdm(receipt) => receipt.rlp_encoded_fields_length_with_bloom(bloom),
        ...
        T: Encodable,
    {
        match self {
            ...
            Self::Eip1559(receipt) |
            Self::Eip7702(receipt) |
            Self::Sdm(receipt) => receipt.rlp_encode_fields_with_bloom(bloom, out),
        ...
        T: Decodable,
    {
        match tx_type {
            ...
            OpTxType::Sdm => {
                let ReceiptWithBloom { receipt, logs_bloom } =
                    RlpDecodableReceipt::rlp_decode_with_bloom(buf)?;
                Ok(ReceiptWithBloom { receipt: Self::Sdm(receipt), logs_bloom })
            }
            OpTxType::Deposit => {
                let ReceiptWithBloom { receipt, logs_bloom } =
                    RlpDecodableReceipt::rlp_decode_with_bloom(buf)?;
        ...
        T: Decodable,
    {
        ...
        match tx_type {
            ...
            OpTxType::Sdm => Ok(Self::Sdm(Receipt { status, cumulative_gas_used, logs })),
...
impl From<super::OpReceiptEnvelope> for OpReceipt {
    fn from(envelope: super::OpReceiptEnvelope) -> Self {
        match envelope {
            ...
            super::OpReceiptEnvelope::Sdm(receipt) => Self::Sdm(receipt.receipt),
...
impl<T> From<ReceiptWithBloom<OpReceipt<T>>> for OpReceiptEnvelope<T> {
    fn from(value: ReceiptWithBloom<OpReceipt<T>>) -> Self {
        ...
        match receipt {
            ...
            OpReceipt::Sdm(receipt) => Self::Sdm(ReceiptWithBloom { receipt, logs_bloom }),
...
pub(crate) mod serde_bincode_compat {
    ...
    pub enum OpReceipt<'a> {
        ...
        /// SDM receipt
        Sdm(alloy_consensus::serde_bincode_compat::Receipt<'a, alloy_primitives::Log>),
        /// Deposit receipt
    ...
    impl<'a> From<&'a super::OpReceipt> for OpReceipt<'a> {
        fn from(value: &'a super::OpReceipt) -> Self {
            match value {
                ...
                super::OpReceipt::Sdm(receipt) => Self::Sdm(receipt.into()),
    ...
    impl<'a> From<OpReceipt<'a>> for super::OpReceipt {
        fn from(value: OpReceipt<'a>) -> Self {
            match value {
                ...
                OpReceipt::Sdm(receipt) => Self::Sdm(receipt.into()),
